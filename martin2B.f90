...
module all_item_lin
	...
	subroutine index_estimated (s00, s11, s10, s01, s20, D, m, sig2_m)
		real (kind = rk), intent (in) :: s00, s11, s10, s01, s20, D
		real (kind = rk), intent (out) :: m,  sig2_m
...
real (kind = rk) :: ms00, ms11, ms10, ms01, ms20, D, mm, msig2_m
...
call all_parameters(x, y, err_y, ms00, ms11, ms10, ms01, ms20, D)
call index_estimated (ms00, ms11, ms10, ms01, ms20, D, mm, msig2_m)

do i = 1, size(x)
yt(i)= mm*x(i) 
dy(i)=(y(i) - yt(i))
	write(unit = 2, fmt = *) x(i), y(i), err_y(i)
	write(unit = 4, fmt = *) y(i), yt(i), dy(i), err_y(i)
end do
