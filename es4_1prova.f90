...
!-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	...
	program es4_1
		...
		integer             :: i, j, nBin, nk, k, n
		real                :: rnd, xmin, xmax, p, aa, p_asp, x
		...
		do i=1,100
			...
			enddo
			call FillHisto(H_b, x)		
			
		enddo
	
	
		do i=1,H_basp%nBin
			k=int(H_basp%BinCenters(i))
			print*, k
			nk=n-k
			aa=(fact(n)/(fact(k)*fact(nk)))*(p**k)*((1-p)**(n-k))
			p_asp=aa*100
			H_basp%counts(i)=nint(p_asp)
			H_basp%errors(i)=sqrt(p_asp*100*(1-p_asp))
		enddo
		...
		call drawHisto_1( d_b, d_basp, d_basp, 2, d_basp, xmin, xmax, counts, d_basp )
